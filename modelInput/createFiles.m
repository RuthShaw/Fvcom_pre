...
for vv = 1:2:length(varargin) - 1
    keyword = lower(varargin{vv});

    if ~ischar(keyword)
        error('incorrect usage createFiles')
    end
    
    switch keyword
        case 'wind'
            windSwitch = varargin{vv + 1};
    end
end
% Base folder location - where do you want your forcing input files to end
% up?
inputConf.base = pwd;
inputConf.outbase = [inputConf.base,'/output/'];
...
inputConf.casename = caseInfo.casename;
...
%inputConf.utmZone = {'UTC'};

% Sponge layer parameters
inputConf.spongeRadius = 100000; % in metres, or -1 for variable
...
inputConf.temperature = 25;
inputConf.salinity = 31;

% Model time ([Y,M,D,h,m,s])

inputConf.startDate = caseInfo.startDateUTC;
inputConf.endDate = caseInfo.endDateUTC;
%inputConf.modelYear = ;
...
elseif strcmpi(inputConf.obcForcing, 'z')
    ...
    here = pwd; % store the current working directory to return later
    %tpxo_dir = which('TMD');    % find the TPXO directory
    tpxo_dir = caseInfo.obc_tides.dirTMD;   % remove TPXO.m
    cd(tpxo_dir)    % go to TPXO directory
    % Location of the TMD model description file
    inputConf.Model = [tpxo_dir,'YS2010/DATA/Model_YS'];
...
% Mobj = read_sms_mesh(...
%     '2dm', fullfile(caseInfo.basedir, 'maps', [inputConf.casename, '.2dm']),...
%     'bath', fullfile(inputConf.base, 'raw_data', [inputConf.casename, '_dep.dat']),...
%     'coordinate', inputConf.coordInput, 'addCoriolis', true);
addCoriolis = true;
Mobj = read_sms_mesh2(...
    '2dm',fullfile(caseInfo.outputDir,[caseInfo.casename,'.2dm']),...
    'bath',fullfile(caseInfo.outputDir,[caseInfo.casename,'_dep.dat']),...
    'coordinate', 'cartesian',...
    'project',true,...
    'addCoriolis', addCoriolis);
[Mobj.lon,Mobj.lat] = myProject(Mobj.x,Mobj.y,'reverse');
Mobj.nativeCoords = 'spherical';
if addCoriolis
    Mobj = add_coriolis(Mobj, 'uselatitude');
end
read_obc_nodes{1,1} = Mobj.read_obc_nodes{1,2};
clear Mobj.read_obc_nodes
...
if exist(fullfile(inputConf.outbase, 'sigma.dat'),'file')
    ...
    Mobj = read_sigma(Mobj, fullfile(inputConf.base, '/output/sigma.dat'));
else
...
%% ??????????????
if exist(fullfile(inputConf.outbase,'ecs_elevtide.nc'),'file')~=2
...
if windSwitch
    ...
    forcing = get_NCEP_forcing(Mobj, [inputConf.startDateMJD, inputConf.endDateMJD], 'varlist', {'uwnd', 'vwnd'});
    forcing.time = inputConf.startDateMJD:1/2:inputConf.endDateMJD;

    [lon, lat]=meshgrid(forcing.lon, forcing.lat);
    [forcing.x, forcing.y] = myProject(lon, lat, 'forward');
    ...
    write_FVCOM_forcing(Mobj, fullfile(inputConf.outbase,inputConf.casename), forcing_interp, 'test forcing', '3.1.0');
    copyfile(fullfile(inputConf.outbase,[inputConf.casename,'_wnd.nc']),caseInfo.outputDir);
end
cd(inputConf.base);
fprintf('All done!\n')